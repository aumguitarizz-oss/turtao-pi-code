...
def _alarm_cb(alarmed):
    return lambda tid, msg: alarmed.append((tid, msg))


def _cbs(recorded, alerted, alarmed):
    return dict(
        record_crop=lambda f, b: recorded.append(b),
        emit_alert=alerted.append,
        emit_alarm=_alarm_cb(alarmed),
    )
...
class TestLoiterMonitor:
    ...
    def test_records_after_half_second_missing_face(self):
        ...
        monitor.update([_person()], True, [], frame, now=100.0, **_cbs(recorded, alerted, alarmed))
        assert recorded == []  # not yet at threshold

        monitor.update([_person()], True, [], frame, now=100.5, **_cbs(recorded, alerted, alarmed))
    ...
    def test_alerts_after_two_seconds_missing_face(self):
        ...
        monitor.update([_person()], True, [], frame, now=100.0, **_cbs(recorded, alerted, alarmed))
        monitor.update([_person()], True, [], frame, now=102.0, **_cbs(recorded, alerted, alarmed))
    ...
    def test_face_resolving_resets_timer(self):
        ...
        monitor.update([person], True, [], frame, now=100.0, **_cbs(recorded, alerted, alarmed))
        # Face resolves before the 0.5s record threshold
        monitor.update(
            [person], True, [_face_in(person["bbox"])], frame, now=100.2,
            **_cbs(recorded, alerted, alarmed),
        )
        # Now missing again from 100.8; still no record yet (0s elapsed in new episode)
        monitor.update([person], True, [], frame, now=100.8, **_cbs(recorded, alerted, alarmed))
        ...
        monitor.update([person], True, [], frame, now=101.3, **_cbs(recorded, alerted, alarmed))
    ...
    def test_person_leaving_frame_clears_state_after_grace_period(self):
        ...
        monitor.update([], True, [], frame, now=101.5, **_cbs(recorded, alerted, alarmed))
        assert recorded == []

        # Same tracker_id reappears — must behave like a fresh episode, not
        # inherit the old (now-dropped) first_missing_at.
        monitor.update([person], True, [], frame, now=200.0, **_cbs(recorded, alerted, alarmed))
        assert recorded == []  # still 0s elapsed in the fresh episode
        monitor.update([person], True, [], frame, now=200.5, **_cbs(recorded, alerted, alarmed))
    ...
    def test_pose_lost_then_regained_clears_stale_timer(self):
        ...
        monitor.update([person], False, [], frame, now=500.0, **_cbs(recorded, alerted, alarmed))
        assert recorded == []
        assert alerted == []
        ...
        monitor.update([person], True, [], frame, now=500.1, **_cbs(recorded, alerted, alarmed))
        assert recorded == []
        assert alerted == []
...
class TestLoiterMonitorAlarmTier:
    """10s alarm tier — separate from (and on top of) the existing 2s quiet
    alert: same tracked person unresolved continuously for 10s+ fires a
    distinct, louder callback carrying the tracker_id, so the caller can
    mark just that person's box and raise a different notification than
    both the quiet 2s log entry and a confirmed-face THREAT."""

    def test_alarms_after_ten_seconds_missing_face(self):
        monitor = LoiterMonitor()
        recorded, alerted, alarmed = [], [], []
        frame = np.zeros((480, 640, 3), dtype=np.uint8)

        monitor.update([_person()], True, [], frame, now=100.0, **_cbs(recorded, alerted, alarmed))
        assert alarmed == []  # only 0s elapsed

        monitor.update([_person()], True, [], frame, now=109.9, **_cbs(recorded, alerted, alarmed))
        assert alarmed == []  # just under 10s

        monitor.update([_person()], True, [], frame, now=110.1, **_cbs(recorded, alerted, alarmed))
        assert len(alarmed) == 1  # fires once, not every tick past threshold
        tid, msg = alarmed[0]
        assert tid == 1
        assert "tracker #1" in msg
        assert monitor.alarmed_ids() == {1}

    def test_face_resolving_clears_alarm(self):
        monitor = LoiterMonitor()
        recorded, alerted, alarmed = [], [], []
        frame = np.zeros((480, 640, 3), dtype=np.uint8)
        person = _person()
        ...
        monitor.update([person], True, [], frame, now=100.0, **_cbs(recorded, alerted, alarmed))
        monitor.update([person], True, [], frame, now=111.0, **_cbs(recorded, alerted, alarmed))
        assert monitor.alarmed_ids() == {1}

        # Face resolves — alarm state must clear immediately, same as the
        # existing record/alert reset.
        monitor.update(
            [person], True, [_face_in(person["bbox"])], frame, now=111.1,
            **_cbs(recorded, alerted, alarmed),
        )
        assert monitor.alarmed_ids() == set()

    def test_alarmed_ids_empty_before_threshold(self):
        monitor = LoiterMonitor()
        recorded, alerted, alarmed = [], [], []
        monitor.update(
            persons=[_person()], pose_present=True, faces=[],
            frame=np.zeros((480, 640, 3), dtype=np.uint8), now=100.0,
            **_cbs(recorded, alerted, alarmed),
        )
        assert monitor.alarmed_ids() == set()
